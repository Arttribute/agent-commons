...
describe("CommonsClient", () => {
  ...
  it("throws CommonsError on non-ok response", async () => {
    ...
    const client = makeClient(fetch);
    await expect(client.agents.get("missing")).rejects.toBeInstanceOf(
      CommonsError,
    );
  ...
  it("accepts successful 204 responses without parsing JSON", async () => {
    ...
    await expect(
      makeClient(fetch).request("DELETE", "/v1/example"),
...
describe("client.agents", () => {
  ...
  it("list with owner — GET /v1/agents?owner=…", async () => {
    ...
    expect(fetch.mock.calls[0][0]).toBe(
      "http://api.test/v1/agents?owner=user-1",
    );
  });
  ...
  it("submitCliToolResult — POSTs the result with client authorization", async () => {
    ...
    await makeClient(fetch).agents.submitCliToolResult(
      "request-1",
      '{"ok":true}',
    );
    const [url, opts] = fetch.mock.calls[0];
...
describe("client.agents persistent computer", () => {
  ...
  it("computer config — GET and PUT /v1/agents/:id/computer/config", async () => {
    ...
    expect(getFetch.mock.calls[0][0]).toBe(
      "http://api.test/v1/agents/a1/computer/config",
    );
    ...
    expect(putFetch.mock.calls[0][0]).toBe(
      "http://api.test/v1/agents/a1/computer/config",
    );
  ...
  it("getComputer — GET /v1/agents/:id/computer", async () => {
    ...
    expect(fetch.mock.calls[0][0]).toBe(
      "http://api.test/v1/agents/a1/computer",
    );
  ...
  it.each([
    ...
    "%s — POST /v1/agents/:id/computer/%s",
    async (method, action) => {
      const fetch = makeFetch({ data: computer });
  ...
  it("resizeComputer — POST /v1/agents/:id/computer/resize", async () => {
    const fetch = makeFetch({ data: computer });
    await makeClient(fetch).agents.resizeComputer("a1", {
  ...
  it("execComputer — POST /v1/agents/:id/computer/exec", async () => {
    ...
    await makeClient(fetch).agents.execComputer("a1", {
      command: "pwd",
      cwd: "/workspace",
    });
    const [url, opts] = fetch.mock.calls[0];
  ...
  it("openComputerBrowser — POST singular browser path", async () => {
    const fetch = makeFetch({ data: {} });
    await makeClient(fetch).agents.openComputerBrowser("a1", {
  ...
  it("deprecated plural helpers resolve through the singleton endpoints", async () => {
    ...
    const listed = await makeClient(listFetch).agents.listComputers("a1", {
      sessionId: "ignored",
    });
    ...
    expect(startFetch.mock.calls[0][0]).toBe(
      "http://api.test/v1/agents/a1/computer/wake",
    );
    expect(JSON.parse(startFetch.mock.calls[0][1].body)).toEqual({
      reason: "compatibility",
    });
    ...
    expect(stopFetch.mock.calls[0][0]).toBe(
      "http://api.test/v1/agents/a1/computer/sleep",
    );
  });
});
...
describe("client.sessions", () => {
  ...
  it("list — GET /v1/sessions/list/:agentId/:initiatorId", async () => {
    ...
    expect(fetch.mock.calls[0][0]).toBe(
      "http://api.test/v1/sessions/list/a1/user-1",
    );
  });

  it("create — POST /v1/sessions", async () => {
    const fetch = makeFetch({ data: session });
    await makeClient(fetch).sessions.create({
      agentId: "a1",
      initiator: "user-1",
    });
    const [url, opts] = fetch.mock.calls[0];
...
describe("client.workflows", () => {
  ...
  it("create — POST /v1/workflows", async () => {
    ...
    await makeClient(fetch).workflows.create({
      name: "WF",
      definition: { nodes: [], edges: [] },
      ownerId: "user-1",
      ownerType: "user",
  ...
  it("execute — POST /v1/workflows/:id/execute", async () => {
    ...
    expect(fetch.mock.calls[0][0]).toBe(
      "http://api.test/v1/workflows/wf1/execute",
    );
    expect(fetch.mock.calls[0][1].method).toBe("POST");
  });
  ...
  it("approveExecution — POST /v1/workflows/:id/executions/:eid/approve", async () => {
    ...
    await makeClient(fetch).workflows.approveExecution("wf1", "ex1", {
      approvalToken: "tok",
    });
    expect(fetch.mock.calls[0][0]).toBe(
      "http://api.test/v1/workflows/wf1/executions/ex1/approve",
    );
  });
  ...
  it("rejectExecution — POST /v1/workflows/:id/executions/:eid/reject", async () => {
    ...
    await makeClient(fetch).workflows.rejectExecution("wf1", "ex1", {
      approvalToken: "tok",
      reason: "No",
    });
    expect(fetch.mock.calls[0][0]).toBe(
      "http://api.test/v1/workflows/wf1/executions/ex1/reject",
    );
  });
});
...
describe("client.skills", () => {
  ...
  it("get — GET /v1/skills/:slug", async () => {
    ...
    expect(fetch.mock.calls[0][0]).toBe(
      "http://api.test/v1/skills/web-research",
    );
  });
  ...
  it("create — POST /v1/skills", async () => {
    const fetch = makeFetch({ data: {} });
    await makeClient(fetch).skills.create({
      slug: "my-skill",
      name: "My Skill",
      description: "d",
      instructions: "i",
...
describe("client.provenance", () => {
  it("loads a session trajectory with an encoded ID", async () => {
    const fetch = makeFetch({ data: { runs: [], events: [], summary: {} } });
    await makeClient(fetch).provenance.session("session/one");
    expect(fetch.mock.calls[0][0]).toBe(
      "http://api.test/v1/provenance/sessions/session%2Fone",
    );
  });

  it("loads workflow and other generalized execution scopes", async () => {
    const fetch = makeFetch({ data: { runs: [], events: [], summary: {} } });
    await makeClient(fetch).provenance.scope("workflow", "execution/one");
    expect(fetch.mock.calls[0][0]).toBe(
      "http://api.test/v1/provenance/scopes/workflow/execution%2Fone",
    );
  });

  it("requests an explicit on-chain anchor without changing capture defaults", async () => {
    const fetch = makeFetch({
      data: { traceId: "trace-1", status: "pending" },
    });
    await makeClient(fetch).provenance.anchor("trace-1");
...
describe("client.memory", () => {
  ...
  it("stats — GET /v1/memory/agents/:agentId/stats", async () => {
    ...
    expect(fetch.mock.calls[0][0]).toBe(
      "http://api.test/v1/memory/agents/a1/stats",
    );
  });

  it("create — POST /v1/memory", async () => {
    const fetch = makeFetch({ data: {} });
    await makeClient(fetch).memory.create({
      agentId: "a1",
      content: "c",
      summary: "s",
    });
...
describe("client.usage", () => {
  ...
  it("getAgentUsage with date range", async () => {
    const fetch = makeFetch({ data: agg });
    await makeClient(fetch).usage.getAgentUsage("a1", {
      from: "2026-01-01",
      to: "2026-03-01",
    });
    const url: string = fetch.mock.calls[0][0];
  ...
  it("reads structured file image URLs and compatibility aliases", async () => {
    const fetch = makeFetch({
      data: {
        ...
        artifacts: [
          { kind: "image", url: "https://files.example/artifact.png" },
        ],
