...
await build({ entryPoints: [join(cliRoot, 'src/bin.ts')], bundle: true, platform: 'node', format: 'cjs', target: 'node22', outfile: 'dist/cli.cjs', define: { __CLI_VERSION__: JSON.stringify(cli.version) }, alias: { '@agent-commons/sdk': join(cliRoot, '../commons-sdk/src/index.ts') } });
...
await copyFile(join(pdfRoot, 'lib/pdf.js/v1.10.100/build/pdf.js'), 'dist/pdf.js/v1.10.100/build/pdf.js');
