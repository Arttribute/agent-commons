...
OAuth provider redirect URLs should point at the frontend callback path, for example `http://localhost:3000/api/oauth/callback/github`.
Providers with both runtime credentials set are encrypted and upserted
automatically when `commons-api` starts; the legacy OAuth seed script is not
