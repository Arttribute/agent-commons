...
export function RichTextRenderer({
  ...
  if (!isRichText(value)) {
    return (
      <div className={cn("space-y-3 text-[15px] leading-7 text-slate-700", className)}>
        {value
          .split(/\n{2,}|\n/)
          .map((line) => line.trim())
          .filter(Boolean)
