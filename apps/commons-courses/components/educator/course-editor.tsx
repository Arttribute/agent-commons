        ...
        {challenges.map((challenge, challengeIndex) => (
          <div key={challenge.id || challengeIndex} className="rounded-lg border border-slate-200 bg-slate-50 p-4">
            ...
            <SandboxConfigEditor
              ...
              onChange={(sandbox) =>
                onChange(updateSkillPackChallenge(skillPack, challengeIndex, { sandbox }))
              }
            />
      ...
      <div className="space-y-3">
        {rows.map((row, index) => (
          <div key={row.id || index} className="grid gap-2 rounded-lg bg-slate-50 p-3 md:grid-cols-[1fr_1fr_auto]">
            ...
            <div className={kind === "tool" ? "md:col-span-3" : "md:col-span-3"}>
              <TextArea
    ...
    <div className="rounded-lg border border-slate-200 p-3">
      ...
      <div className="space-y-3">
        {steps.map((step, index) => (
          <div key={step.id || index} className="grid gap-2 rounded-lg bg-slate-50 p-3 md:grid-cols-[1fr_180px_auto]">
            <Field
              label="Title"
              value={step.title}
              onChange={(title) => {
...
function createSandboxConfig(): AgentSandboxConfig {
  return {
    enabled: true,
    mode: "builder",
    title: "Create your first agent",
    brief: "Build a small agent with a system prompt, a reusable skill, and one meaningful connected tool.",
    capabilities: [
      "identity",
      "system_prompt",
      "skills",
    ...
    guideSteps: [
      {
        ...
        body: "Give the agent a clear identity so you know what kind of helper you are building.",
      },
      {
        id: "prompt",
        target: "system_prompt",
        title: "Shape behavior",
        body: "Edit the system prompt so the agent has boundaries, tone, and a specific job.",
      },
      {
        id: "skills",
        target: "skills",
        title: "Add a skill",
        body: "Choose a reusable instruction set that helps the agent repeat good practice.",
      },
      {
        id: "tools",
        target: "tools",
        title: "Connect a tool",
        body: "Select a tool or connector so the agent can do more than write a response.",
      },
      {
        id: "run",
        target: "chat",
        title: "Run it",
        body: "Send the first task and inspect the logs for success, warnings, or failure.",
      },
    ],
  };
}
