...
export function CourseMaterialsLibrary({ slug }: { slug: string }) {
  ...
  useEffect(() => {
    fetch(`/api/courses/${slug}/materials`, { cache: "no-store" })
      ...
      .then(({ ok, body }) => {
        if (ok) {
          const nextMaterials = body.materials || [];
          ...
          setSelected((current) =>
            current ||
            (nextMaterials[0]
              ? { kind: "material", id: nextMaterials[0].id }
              : nextWorkspaces[0]
                ? { kind: "lab", id: nextWorkspaces[0].id }
                : null),
          );
          setNotice("");
