...
function makeDb() {
  ...
  const insertReturning = jest
    .fn()
    .mockResolvedValue([{ executionId: 'exec-hitl', status: 'running' }]);
...
describe('HITL integration — WorkflowExecutorService', () => {
  ...
  describe('approveExecution()', () => {
    it('resumes execution after valid approval, walker runs to completion', async () => {
      db.query.workflowExecution.findFirst = jest
        .fn()
        .mockResolvedValue(db.pausedExecution());
    ...
    it('passes approvalData into the resumed node outputs', async () => {
      ...
      expect(capturedResumeOutputs['n-approval']?.approvalData?.reviewer).toBe(
        'alice',
      );
    });
    ...
    it('throws when approval token is wrong', async () => {
      ...
      await expect(
        service.approveExecution('exec-hitl', 'wrong-token'),
  ...
  describe('executeApprovalNode()', () => {
    it('throws HumanApprovalPauseError synchronously when invoked by the walker', async () => {
      ...
      const executeApprovalNode = (service as any).executeApprovalNode?.bind(
        service,
      );
