...
import * as wavfile from 'wavefile'

import type { Embedding } from '../types/embedding.type.js'
import { type CreateEmbedding, EmbeddingType } from '../types/embedding.type.js'
...
export class EmbeddingService {
  ...
  private async embed(content: string, type: EmbeddingType): Promise<number[]> {
    ...
    switch (type) {
      case EmbeddingType.text: {
        ...
        const inputs = tokenizer([content], { padding: true, truncation: true })
      ...
      case EmbeddingType.audio: {
        ...
        const wav = new wavfile.WaveFile(buffer)
        wav.toBitDepth('32f')
      ...
      default:
        throw new HTTPException(400, { message: 'Unsupported resource type' })
    }

    return Array.from(output.data)
  }

  async createEmbedding(dto: CreateEmbedding) {
    const { content, resourceType, embeddingType, schema, tags, resourceFile } =
      dto

    if (!content || !embeddingType)
      throw new HTTPException(400, {
        message: 'Content embedding type are required',
      })
    ...
    const {
      data,
      error,
    }: { data: Embedding | null; error: PostgrestError | null } =
      await this.supabase
        .from('resource')
  ...
  async find(
    dto: Pick<CreateEmbedding, 'content' | 'embeddingType'>,
    ...
    if (!content || !embeddingType)
      throw new HTTPException(400, {
        message: 'Content and embeddingType are required',
      })
