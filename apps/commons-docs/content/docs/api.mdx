...
Use a project API key from the
[Commons API Platform](https://auth.agentcommons.io/platform), or a short-lived
Commons OAuth access token:

```http
Authorization: Bearer csk_live_your_project_key
Content-Type: application/json
```

Project keys belong to a workspace project and can be scoped independently.
Legacy `sk-ac-*` keys remain accepted during migration.
...
## Compute and Common OS

Common OS remains an independently deployed service and is exposed through the
same credential and API platform under `/v1/compute`.

| Method | Path | Description |
|---|---|---|
| POST | /v1/compute/fleets | Create a fleet |
| GET | /v1/compute/fleets | List fleets |
| GET | /v1/compute/fleets/:fleetId | Get a fleet |
| POST | /v1/compute/fleets/:fleetId/agents | Deploy an Agent Commons agent |
| GET | /v1/compute/fleets/:fleetId/agents | List deployed agents |
| GET | /v1/compute/fleets/:fleetId/agents/:agentId | Get a deployment |
| DELETE | /v1/compute/fleets/:fleetId/agents/:agentId | Terminate a deployment |
| GET | /v1/compute/activity/events | Query deployment activity |

Passing an existing Agent Commons `agentId` deploys that agent to compute
without creating a duplicate agent identity.
