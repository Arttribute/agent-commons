  ...
  return (
    <div className="space-y-3">
      <label className="block text-xs text-muted-foreground">
        Funding network
        <select
          aria-label="Funding network"
          className="mt-1 w-full rounded-md border border-border bg-background p-2 text-sm"
          value={chainId}
          onChange={(e) => setChainId(e.target.value)}
        >
          {WALLET_NETWORKS.filter((n) => n.testnet).map((n) => (
      ...
      <p className="text-xs text-muted-foreground">
        ...
        <span className="font-medium">{network.name}</span> to fund the agent.
      </p>

      <p className="text-xs text-muted-foreground">
        The agent also needs {network.symbol} for network fees.
        {chainId === "296"
          ? " Associate the USDC token before funding Hedera."
          : ""}{" "}
        Copying an address does not transfer funds.
      </p>
      <a
        href={`${network.explorer}/${chainId === "296" ? "account" : "address"}/${wallet.address}`}
        target="_blank"
