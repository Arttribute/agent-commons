...
export function CanvasStudio({ artifactId }: { artifactId: string }) {
  ...
  const annotationContext = useRef<Promise<unknown> | null>(null);
  const recordingFiles = useRef<Record<string, string>>({});
  async function saveRecording(recording: CanvasRecording) {
    try {
      const stream = new Blob([JSON.stringify(recording)])
        .stream()
        .pipeThrough(new CompressionStream("gzip"));
      const blob = await new Response(stream).blob();
      if (blob.size > 4 * 1024 * 1024)
        throw new Error(
          "This recording is too large for Library upload. Use Download to keep it.",
        );
      const form = new FormData();
      form.append(
        "files",
        new File([blob], `${recording.id}.commons-recording.json.gz`, {
      ...
      if (!response.ok || !result.data?.[0]?.fileId)
        throw new Error(
          result.message ||
  ...
  useEffect(() => {
    let cancelled = false;
    void Promise.all([
      fetch("/api/canvas/models", { cache: "no-store" }).then(
        async (response) => {
          const payload = await response.json().catch(() => null);
      ...
      .catch((cause) => {
        if (!cancelled) {
          setError(
            cause instanceof Error ? cause.message : "Canvas could not open",
          );
        }
      })
      .finally(() => {
  ...
  useEffect(() => {
    ...
    fetch(
      ...
      .then(async (response) => {
        ...
        if (!response.ok) {
          throw new Error(
            payload?.message || "Could not preview this revision",
          );
        }
      ...
      models.find(
        (entry) =>
          ...
          !entry.badges?.some((badge) =>
            badge.toLowerCase().includes("retire"),
          ),
      ) ??
  ...
  useEffect(() => {
    ...
    const timeout = window.setTimeout(() => {
      ...
      if (operation === "transform" && bundle.project.activeItemId)
        inputItemIds.unshift(bundle.project.activeItemId);
      void fetch("/api/canvas/quote", {
        ...
        .then(async (response) => {
          const payload = await response.json().catch(() => null);
          if (!response.ok)
            throw new Error(payload?.message || "Quote unavailable");
  ...
  const annotationDraftOpen =
    Boolean(draftRect || draftPoint) || annotationTool === "time_range";
  const timeline = (bundle?.project.settings?.timeline ??
    null) as CanvasTimeline | null;
  ...
  const contentBox = useMemo<ContentBox>(
    () => fitContentBox(stageSize, intrinsicSize),
  ...
  async function generate() {
    ...
    if (!response.ok) {
      setGenerating(false);
      setError(
        payload?.message || payload?.error || "Could not start generation",
      );
      return;
    }
  ...
  function onStagePointerDown(event: ReactPointerEvent<HTMLDivElement>) {
    ...
    if (temporal && mediaRef.current) mediaRef.current.pause();
  ...
  async function saveAnnotation() {
    ...
    if (draftRect) {
      ...
      endMs = Math.round(
        Math.min(durationMs || currentTimeMs + 5_000, currentTimeMs + 5_000),
      );
    }
    ...
    const compiledContext = isCode
      ? await annotationContext.current
      : undefined;
    ...
    const response = await fetch(
      `/api/canvas/projects/${encodeURIComponent(bundle.project.projectId)}/annotations`,
      {
        ...
        body: JSON.stringify({
          ...
          metadata: {
            schemaVersion: 2,
            intrinsicSize: isCode
              ? { width: 1280, height: 720 }
              : intrinsicSize,
            compiledContext,
            recordingFiles: isCode ? recordingFiles.current : undefined,
            coordinateSpace: geometry ? "artifact_content" : undefined,
            frameTimeMs:
              geometry && temporal ? Math.round(currentTimeMs) : undefined,
          },
        }),
      },
    );
  ...
  const estimatedCost =
    quote?.estimatedCostUsd ??
    (model ? estimateCost(model, settings, prompt) : 0);
  ...
  const sourceFiles =
    preview?.codeProject?.files ??
    (preview?.content
      ? [{ path: preview.name, content: preview.content }]
      : []);
  const temporal =
      ...
      <div className="flex min-h-0 flex-1">
        {leftOpen ? (
          <ResizablePanel
            side="left"
            defaultWidth={310}
            label={isCode ? "Code project" : "Creative tools"}
            className="flex shrink-0 flex-col border-r border-stone-200 bg-white"
          >
            {isCode ? (
              <div className="space-y-4 p-4">
                <div className="flex items-center justify-between">
                  <h2 className="text-xs font-semibold">Code project</h2>
                  <button
                ...
                <p className="text-xs leading-5 text-stone-500">
                  {preview?.codeProject?.name || preview?.name}
                </p>
                <button
                  className="flex w-full items-center gap-2 rounded-lg border border-stone-200 p-3 text-xs"
                  onClick={() => setStageView("code")}
                >
                  <Code2 className="h-4 w-4" />
                  Explore {sourceFiles.length} source files
                </button>
                <p className="text-xs leading-5 text-stone-500">
                  Use the copilot to change the code or add images, audio and
                  other assets. Attach reference files in the conversation.
                </p>
                <button
                  className="flex w-full items-center gap-2 rounded-lg border border-stone-200 p-3 text-xs"
                  onClick={() => askCopilot()}
              ...
              <>
                    ...
                    <div className="mt-2 rounded-lg bg-stone-50 p-3">
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <span className="text-right text-[10px] text-stone-500">
                          {quote
                            ? `~${quote.estimatedCredits} credits`
                      ...
                      <p className="mt-1 text-[9px] leading-3 text-stone-400">
                        ${estimatedCost.toFixed(4)} estimate ·{" "}
                        {model.pricing.settlement === "provider_usage"
                      ...
                      {model.badges?.length ? (
                        <div className="mt-2 flex flex-wrap gap-1">
                          {model.badges.map((badge) => (
                            <span
                              key={badge}
                              className="rounded-full border border-stone-200 bg-white px-2 py-0.5 text-[9px] text-stone-500"
                            >
                              {badge}
                            </span>
                          ))}
                ...
                <div className="border-t border-stone-200 bg-white p-3">
                  {displayedJob &&
                  ["queued", "running"].includes(displayedJob.status) ? (
                    <JobProgress job={displayedJob} />
                  ) : displayedJob?.status === "failed" ? (
                    <p className="mb-2 line-clamp-2 text-[10px] text-rose-600">
        ...
        <main className="flex min-w-0 flex-1 flex-col">
          <div className="flex h-11 shrink-0 items-center justify-center gap-1 border-b border-stone-200 bg-white/80 px-3 backdrop-blur">
            {isCode && (
              <div className="mr-3 flex items-center rounded-lg bg-stone-100 p-0.5">
                {(["preview", "code"] as const).map((mode) => (
                  <button
                    key={mode}
                    type="button"
                    onClick={() => setStageView(mode)}
                    aria-pressed={stageView === mode}
                    className={cn(
            ...
            <CanvasTool
              ...
              label={isCode ? "Interact with artifact" : "Select"}
              onClick={() => setAnnotationTool("select")}
            >
              <MousePointer2 />
            </CanvasTool>
            <CanvasTool
              active={annotationTool === "region"}
              label="Region annotation"
              onClick={() => setAnnotationTool("region")}
            >
              <Scan />
            </CanvasTool>
            <CanvasTool
              active={annotationTool === "point"}
              label="Point annotation"
              onClick={() => setAnnotationTool("point")}
            >
              <MapPin />
            </CanvasTool>
            {temporal ? (
              <CanvasTool
                active={annotationTool === "time_range"}
                label="Time note"
                onClick={() => {
                  setAnnotationTool("time_range");
                  setRightOpen(true);
                  setRightPanel("notes");
                }}
              >
            ...
            <button
              type="button"
              onClick={() => setZoom(1)}
              className="min-w-12 rounded-md px-2 py-1 text-[10px] text-stone-500 hover:bg-stone-100"
            >
              {Math.round(zoom * 100)}%
            </button>
            <CanvasTool
          ...
          <div className="relative flex min-h-0 flex-1 items-center justify-center overflow-auto p-6 lg:p-8">
            <div
              ...
              ref={stageRef}
            >
              {isCode && stageView === "code" ? (
                <CodeFileBrowser files={sourceFiles} />
              ) : previewLoading ? (
                ...
                <ArtifactStage
                  ...
                  onRecording={(recording) => void saveRecording(recording)}
            ...
            {!leftOpen ? (
              <button
                ...
                className="absolute left-3 top-3 flex h-9 w-9 items-center justify-center rounded-lg border border-stone-200 bg-white text-stone-500 shadow-sm hover:text-stone-900"
                aria-label="Show creative controls"
              >
            ...
            {!rightOpen ? (
              <button
                ...
                className="absolute right-3 top-3 flex h-9 w-9 items-center justify-center rounded-lg border border-stone-200 bg-white text-stone-500 shadow-sm hover:text-stone-900"
              ...
              <TemporalTimeline
                ...
                onAddClip={(item) =>
                  void timelineAction({
                    action: "add_clip",
                    itemId: item.itemId,
                    trackKind: item.mimeType.startsWith("audio/")
                      ? "audio"
                      : "video",
                    durationMs:
                      item.itemId === bundle.project.activeItemId
                        ? durationMs
                        : 5_000,
                  })
                }
        ...
        {rightOpen ? (
          <ResizablePanel
            ...
            className="flex shrink-0 border-l border-stone-200 bg-white"
          >
            <div className="flex w-12 shrink-0 flex-col items-center gap-1 border-r border-stone-200 py-2">
              ...
              <RailButton
                ...
                label="Notes"
                badge={
                  activeAnnotations.filter((item) => item.status === "open")
                    .length
                }
            ...
            <div className="flex min-w-0 flex-1 flex-col">
              <div className="flex h-12 shrink-0 items-center border-b border-stone-200 px-4">
                <h2 className="text-xs font-semibold capitalize text-stone-900">
                  {rightPanel}
                </h2>
                <button
                  type="button"
                ...
                {rightPanel === "project" ? (
                  ...
                  <NotesPanel
                    ...
                    onResolve={(annotation) =>
                      void resolveAnnotation(annotation)
                    }
                    onAsk={askCopilot}
                  />
                ) : (
...
export function ModelSelector({
  ...
  const SelectedIcon =
    KIND_OPTIONS.find((entry) => entry.kind === selected?.kind)?.icon ??
    Sparkles;
      ...
      >
        ...
        <span className="min-w-0 flex-1">
          ...
          <span className="block truncate text-[9px] capitalize text-stone-400">
            {selected
              ? `${selected.provider} · ${selected.kind} · ${selected.tier}`
              : "Search every media provider"}
          </span>
        </span>
        <ChevronDown
          className={cn(
                    ...
                    >
                      <span
                        className={cn(
                          ...
                          entry.available ? "bg-emerald-500" : "bg-stone-300",
                        )}
                      />
                      <span className="min-w-0 flex-1">
                        <span className="flex items-center justify-between gap-2">
                          ...
                          <span className="shrink-0 text-[9px] capitalize text-stone-400">
                            {entry.provider}
                          </span>
                        </span>
                        <span className="mt-0.5 block line-clamp-2 text-[9px] leading-3 text-stone-500">
                          {entry.description}
                        </span>
                        {!entry.available ? (
                          <span className="mt-1 block text-[8px] text-amber-700">
                            {entry.unavailableReason === "price_not_configured"
...
function ArtifactStage({
  ...
  interactive?: boolean;
  compiledRef?: React.Ref<CompiledFrameHandle>;
  ...
  if (!source) {
    return (
      <div className="max-w-lg whitespace-pre-wrap p-8 text-sm text-stone-200">
        {preview.content ||
          preview.textPreview ||
  ...
  if (preview.mimeType.startsWith("audio/")) {
    return (
      <div className="flex w-full max-w-xl flex-col items-center px-10 text-center text-white">
        ...
        <p className="mt-5 max-w-full truncate text-sm font-medium">
          {preview.name}
        </p>
...
function AnnotationOverlay({
  ...
  const geometry = annotation.geometry as Partial<
  ...
  if (
    ...
    return (
      <span
        className="absolute flex h-6 w-6 -translate-x-1/2 -translate-y-full items-center justify-center rounded-full border-2 border-white bg-amber-500 text-[9px] font-bold text-white shadow"
  ...
  if (
    ...
    return (
      <span
        className={cn(
          ...
          annotation.status === "resolved"
            ? "border-emerald-400/70 bg-emerald-300/10"
      ...
      >
        <span className="absolute -right-3 -top-3 flex h-6 w-6 items-center justify-center rounded-full border-2 border-white bg-amber-500 text-[9px] font-bold text-white shadow">
          {index}
        </span>
      </span>
    );
...
function DraftRect({ rect }: { rect: Rect }) {
  return (
    <span
      className="absolute border-2 border-cyan-400 bg-cyan-300/10"
      style={{
        left: `${rect.x * 100}%`,
        top: `${rect.y * 100}%`,
...
function DraftPoint({ point }: { point: { x: number; y: number } }) {
  return (
    <span
      className="absolute h-7 w-7 -translate-x-1/2 -translate-y-full rounded-full border-2 border-white bg-cyan-500 shadow"
      ...
      {draftOpen ? (
        <div className="mb-3 rounded-xl border border-cyan-200 bg-cyan-50/60 p-3">
          <div className="flex items-center gap-2 text-[11px] font-semibold text-cyan-900">
            ...
            {annotationTool === "time_range"
              ? `Note at ${formatCanvasTime(currentTimeMs)}`
              : "Add annotation"}
          </div>
          <Textarea
            autoFocus
            ...
            placeholder="What should collaborators or agents understand here?"
            className="mt-2 min-h-[88px] resize-none border-cyan-200 bg-white text-xs"
          />
          <div className="mt-2 flex justify-end gap-1.5">
            ...
            <Button
              type="button"
              size="sm"
              className="h-8 gap-1.5 bg-stone-900 text-xs"
        ...
        <div className="mb-3 rounded-lg border border-dashed border-stone-200 p-3 text-[10px] leading-4 text-stone-500">
          Choose Region, Point, or Time note above to give humans and agents
          exact context.
        </div>
      )}
        ...
        {annotations.length ? (
          annotations.map((annotation, index) => (
            <article
              ...
              onDragStart={(event) => {
                const context = {
                  ...
                  target: annotationLocation(annotation),
                };
                event.dataTransfer.setData(
                  "application/x-agent-commons-canvas-context",
                  JSON.stringify(context),
                );
                event.dataTransfer.setData(
                  "text/plain",
                  `Canvas annotation: ${annotation.body}\n${annotationLocation(annotation)}`,
                );
              }}
              className={cn(
            ...
            >
                ...
                <div className="min-w-0 flex-1">
                  <p
                    className={cn(
                      ...
                      annotation.status === "resolved" && "line-through",
                    )}
                  >
                    {annotation.body}
                  </p>
                  <p className="mt-1 text-[9px] text-stone-400">
                    {annotationLocation(annotation) || "Whole revision"} ·{" "}
              ...
              <div className="mt-2 flex justify-end gap-1 opacity-0 transition group-hover:opacity-100">
                <button
                  type="button"
                  onClick={() => onAsk(annotation)}
                  className="rounded-md px-2 py-1 text-[9px] text-stone-500 hover:bg-stone-100 hover:text-stone-900"
                >
                  Ask Copilot
                </button>
                <button
          ...
          <div className="py-10 text-center">
            ...
            <p className="mt-1 text-[10px] text-stone-400">
              Mark a precise place or moment.
            </p>
          </div>
        )}
      </div>
    ...
    <div className="p-3">
      <div className="rounded-xl border border-stone-200 bg-stone-50 p-3">
        ...
        <p className="mt-1 text-[10px] text-stone-500">
          {bundle.revisions.length} revision
          {bundle.revisions.length === 1 ? "" : "s"} ·{" "}
          {bundle.annotations.length} note
          {bundle.annotations.length === 1 ? "" : "s"}
        </p>
      </div>
      <div className="mt-3 grid grid-cols-2 gap-2">
        <button
          ...
          onClick={onUpload}
          disabled={uploading}
          className="flex h-9 items-center justify-center gap-1.5 rounded-lg border border-stone-200 text-[10px] font-medium text-stone-600 hover:bg-stone-50 disabled:opacity-50"
        >
          {uploading ? (
        ...
        <button
          type="button"
          onClick={onLibrary}
          className="flex h-9 items-center justify-center gap-1.5 rounded-lg border border-stone-200 text-[10px] font-medium text-stone-600 hover:bg-stone-50"
        >
        ...
        {bundle.revisions.map((revision) => (
          <button
            ...
            className={cn(
              ...
              revision.revisionId === activeRevision?.revisionId
                ? "border-stone-300 bg-stone-100"
                : "border-transparent hover:bg-stone-50",
            )}
          >
            <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-md bg-white text-stone-500 shadow-sm">
              <ArtifactKindIcon kind={revision.artifact?.kind} />
            </span>
            <span className="min-w-0 flex-1">
              <span className="block truncate text-[11px] font-medium text-stone-700">
                {revision.artifact?.name || "Artifact revision"}
              </span>
              <span className="block truncate text-[9px] capitalize text-stone-400">
                {revision.operation} ·{" "}
                {revision.modelId || revision.createdByType}
              </span>
            </span>
            {revision.revisionId === activeRevision?.revisionId ? (
              ...
              .filter(
                (asset) =>
                  !bundle.revisions.some(
                    (revision) => revision.itemId === asset.itemId,
                  ),
              )
              .map((asset) => (
                <div
                  key={asset.itemId}
                  className="flex items-center gap-2 rounded-lg px-2.5 py-2 hover:bg-stone-50"
                >
                  <span className="flex h-7 w-7 items-center justify-center rounded-md bg-stone-100 text-stone-500">
                    <ArtifactKindIcon kind={asset.kind} />
                  </span>
...
function HistoryPanel({
  ...
  const jobsByOutput = new Map(
    jobs
      .filter((job) => job.outputItemId)
      .map((job) => [job.outputItemId, job]),
  );
  return (
    <div className="p-4">
      <div className="mb-4 rounded-xl border border-emerald-200 bg-emerald-50 p-3">
        <div className="flex items-center gap-2 text-[11px] font-semibold text-emerald-800">
          <GitBranch className="h-3.5 w-3.5" /> Provenance active
        </div>
        <p className="mt-1 text-[9px] leading-4 text-emerald-700">
          Every model action creates a traceable, non-destructive revision.
          Prompts are stored privately and exposed here only by hash.
        </p>
      </div>
          ...
          return (
            <div key={revision.revisionId} className="relative flex gap-3 pb-5">
              ...
              <div className="min-w-0">
                <p className="truncate text-[11px] font-medium capitalize text-stone-700">
                  {revision.operation} {revision.artifact?.kind || "artifact"}
                </p>
                <p className="mt-0.5 truncate text-[9px] text-stone-400">
                  {revision.modelId || "Imported"} ·{" "}
                  {relativeDate(revision.createdAt)}
                </p>
                {revision.promptHash ? (
                  <p className="mt-1 truncate font-mono text-[8px] text-stone-400">
                    {revision.promptHash.slice(0, 24)}…
                  </p>
                ) : null}
                {job?.actualCostUsd ? (
                  <p className="mt-1 text-[9px] text-stone-400">
                    ${Number(job.actualCostUsd).toFixed(3)} · {job.status}
                  </p>
                ) : null}
              </div>
            </div>
      ...
      <div className="flex min-w-0 flex-1 items-center gap-2 overflow-x-auto px-4 py-3">
        {[...revisions].reverse().map((revision, index) => (
          <button
            ...
            onClick={() => onSelect(revision)}
            className={cn(
              "group flex h-[88px] w-40 shrink-0 flex-col overflow-hidden rounded-lg border bg-white text-left transition",
              revision.itemId === activeItemId
                ? "border-stone-800 ring-1 ring-stone-800"
                : "border-stone-200 hover:border-stone-400",
            )}
          >
            <span className="flex min-h-0 flex-1 items-center justify-center bg-stone-100 text-stone-400">
              <ArtifactKindIcon
            ...
            <span className="flex h-7 items-center gap-1.5 px-2">
              ...
              <span className="min-w-0 flex-1 truncate text-[8px] capitalize text-stone-400">
                {revision.operation} · {revision.modelId || "source"}
              </span>
            </span>
          </button>
        ))}
    ...
    <div className="flex h-full flex-col">
      <div className="flex h-9 items-center gap-2 border-b border-stone-100 px-3">
        <button
          type="button"
          onClick={onToggle}
          className="flex h-7 w-7 items-center justify-center rounded-full bg-stone-900 text-white"
        >
          {playing ? (
        ...
        <span className="font-mono text-[10px] text-stone-600">
          {formatCanvasTime(currentTimeMs)} / {formatCanvasTime(durationMs)}
        </span>
        ...
        {!clips.length && currentAsset ? (
          <button
            type="button"
            onClick={() => onAddClip(currentAsset)}
        ...
        <button
          ...
          onClick={onSplit}
          className="flex h-7 items-center gap-1 rounded-md px-2 text-[9px] text-stone-500 hover:bg-stone-100 disabled:opacity-30"
        >
        ...
        <button
          ...
          disabled={!selectedClipId}
          onClick={onDelete}
          className="flex h-7 items-center gap-1 rounded-md px-2 text-[9px] text-rose-600 hover:bg-rose-50 disabled:opacity-30"
        >
        ...
        <div className="ml-auto flex rounded-md bg-stone-100 p-0.5">
          {(["timeline", "storyboard"] as const).map((entry) => (
            <button
              ...
              className={cn(
                "rounded px-2 py-1 text-[8px] capitalize",
                view === entry
      ...
      <div className="relative min-h-0 flex-1 overflow-auto px-4 py-2">
        <div className="mb-2 flex justify-between text-[8px] text-stone-400">
          <span>0:00</span>
          <span>{formatCanvasTime(durationMs / 2)}</span>
          <span>{formatCanvasTime(durationMs)}</span>
        </div>
        {view === "storyboard" && clips.length ? (
          <div className="flex gap-2 overflow-x-auto pb-1">
            {clips.map((clip, index) => (
              <button
                ...
                className={cn(
                  "flex h-12 w-28 shrink-0 flex-col justify-center rounded-lg border px-2 text-left",
                  selectedClipId === clip.clipId
                    ? "border-stone-900 bg-stone-100"
                    : "border-stone-200 bg-white",
                )}
              >
                <span className="truncate text-[9px] font-medium text-stone-700">
                  {index + 1}. {clip.name}
                </span>
                <span className="mt-1 text-[8px] text-stone-400">
                  {formatCanvasTime(clip.durationMs)}
                </span>
              </button>
            ))}
          </div>
          ...
          <div className="relative min-h-12 space-y-1 rounded-lg bg-stone-100 p-1">
            <div
              className="absolute inset-y-0 left-0 rounded-l-lg bg-stone-200"
              style={{ width: `${(currentTimeMs / duration) * 100}%` }}
            />
            {tracks.map((track) => (
              <div key={track.trackId} className="relative h-5">
                {track.clips.map((clip) => (
                  <button
                    ...
                    className={cn(
                      "absolute inset-y-0 overflow-hidden rounded border px-1 text-left text-[8px]",
                      track.kind === "audio"
                        ? "border-violet-400 bg-violet-200 text-violet-900"
                        : "border-cyan-500 bg-cyan-200 text-cyan-900",
                      selectedClipId === clip.clipId &&
                        "ring-2 ring-stone-900 ring-offset-1",
                    )}
                    style={{
                      left: `${(clip.startMs / duration) * 100}%`,
                      width: `${Math.max(1, (clip.durationMs / duration) * 100)}%`,
                    }}
                  >
            ...
            {annotations
              .filter(
                (annotation) =>
                  annotation.startMs !== null &&
                  annotation.startMs !== undefined,
              )
              .map((annotation) => (
                <span
                  key={annotation.annotationId}
                  ...
                  style={{
                    left: `${((annotation.startMs ?? 0) / duration) * 100}%`,
                  }}
                />
              ))}
            <input
            ...
            />
            <span
              className="pointer-events-none absolute inset-y-0 w-px bg-stone-900"
  ...
  return (
    <div className="mb-2 rounded-lg bg-stone-50 p-2.5">
      <div className="flex items-center justify-between text-[10px]">
        ...
        <span className="font-mono text-stone-400">{job.progress}%</span>
      </div>
...
function HeaderButton({
  ...
  return (
    <button
      ...
      onClick={onClick}
      disabled={disabled}
      className="flex h-9 w-9 items-center justify-center rounded-lg text-stone-500 transition hover:bg-stone-100 hover:text-stone-900 disabled:cursor-not-allowed disabled:opacity-35 [&_svg]:h-4 [&_svg]:w-4"
    >
      {children}
    </button>
  );
...
function CanvasTool({
  ...
  return (
    <CanvasToolButton active={active} label={label} onClick={onClick}>
      {children}
    </CanvasToolButton>
  );
}
...
function RailButton({
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={cn(
        "relative flex h-10 w-10 items-center justify-center rounded-lg transition [&_svg]:h-4 [&_svg]:w-4",
        active
    ...
    >
      {children}
      {badge ? (
        <span className="absolute right-0 top-0 flex h-4 min-w-4 items-center justify-center rounded-full bg-amber-500 px-1 text-[8px] font-bold text-white">
          {badge}
        </span>
      ) : null}
    </button>
  ...
  return (
    <div className="flex h-full items-center justify-center bg-stone-100">
      <div className="text-center">
        ...
        <Loader2 className="mx-auto mt-5 h-4 w-4 animate-spin text-stone-400" />
        <p className="mt-2 text-xs text-stone-500">Opening Canvas…</p>
      </div>
    </div>
  );
}
...
function estimateCost(
  ...
  if (model.pricing.unit === "second")
    return model.pricing.usd * Number(settings.durationSeconds ?? 8);
...
function promptPlaceholder(kind: MediaKind, operation: MediaOperation) {
  if (operation === "transform")
    return "Describe exactly what should change, and what must stay the same…";
  if (kind === "video")
    return "Describe the scene, action, camera movement, light, dialogue, and sound…";
  if (kind === "audio")
    return "Write the words to speak and describe the desired delivery…";
  if (kind === "music")
    return "Describe genre, mood, instrumentation, structure, and energy…";
...
function annotationLocation(annotation?: CanvasAnnotation) {
  ...
  if (annotation.startMs !== undefined && annotation.startMs !== null) {
    const end =
      annotation.endMs !== undefined && annotation.endMs !== null
        ? `–${formatCanvasTime(annotation.endMs)}`
        : "";
  ...
  if (geometry?.x !== undefined && geometry.y !== undefined) {
    if (annotation.kind === "region")
      return `region x ${percent(geometry.x)}, y ${percent(geometry.y)}, w ${percent(geometry.width ?? 0)}, h ${percent(geometry.height ?? 0)}`;
