...
export async function GET(
  ...
  const session = await fetch(`${baseUrl}/v1/sessions/${encodeURIComponent(sessionId)}`, {
    cache: "no-store",
    headers: await backendAuthHeaders(),
  });
  const sessionBody = await session.json().catch(() => ({}));
  if (!session.ok) return NextResponse.json(sessionBody, { status: session.status });
  const owner = sessionBody?.data?.initiator;
  if (owner && owner.toLowerCase() !== user.userId.toLowerCase()) {
    return NextResponse.json({ error: "Forbidden" }, { status: 403 });
  }
  const result = await fetch(`${baseUrl}/v1/provenance/sessions/${encodeURIComponent(sessionId)}`, {
    cache: "no-store",
    headers: await backendAuthHeaders(),
  });
  return NextResponse.json(await result.json().catch(() => ({})), { status: result.status });
}
