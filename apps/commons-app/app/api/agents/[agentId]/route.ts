...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ agentId: string }> }) {
  ...
  try {
    const res = await fetch(`${baseUrl}/v1/agents/${agentId}`, { cache: "no-store", headers: await backendAuthHeaders() });
...
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ agentId: string }> }) {
  ...
  if (!baseUrl) return NextResponse.json({ error: "Server base URL not configured" }, { status: 500 });
  try {
    const res = await fetch(`${baseUrl}/v1/agents/${agentId}`, { method: "DELETE", headers: await backendAuthHeaders() });
    const data = await res.json().catch(() => ({ error: "Bad JSON" }));
    return NextResponse.json(data, { status: res.status });
  } catch (e: any) {
    return NextResponse.json({ error: e.message }, { status: 500 });
